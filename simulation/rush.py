...
def run(city_name):
    ...
    parser.add_argument(
        "--config_file", type=str, default=f"./dataset_rush/{city_name}/config.json", help="path of config file")

    parser.add_argument(
        "--engine_config_file", type=str, default=f"./dataset_rush/{city_name}/engine.cfg", help="path of config file")
    ...
    json_cfg = {"dir": f"dataset_rush/{city_name}/",
        "roadnetFile": "roadnet.txt",
	    "flowFile": f"flow{args.rate}.txt",
	    "routeFile": "None.json"
    }
    json_cfg_str = json.dumps(json_cfg, indent=4)
    with open('./dataset_rush/' + city_name + '/config.json', 'w') as json_file:
        json_file.write(json_cfg_str)
    
    conf1 = configparser.ConfigParser()
    conf2 = configparser.ConfigParser(delimiters=':')
    cfg_file = open('./dataset_rush/' + city_name + '/engine.cfg', 'w')
    conf1.set(None, 'start_time_epoch', '0')
    conf1.set(None, 'max_time_epoch', '3600')
    conf2.set(None, 'road_file_addr', f'./dataset_rush/{city_name}/roadnet.txt')
    conf2.set(None, 'vehicle_file_addr', f'./dataset_rush/{city_name}/flow{args.rate}.txt')
    ...
    with open('./dataset_rush/' + city_name + '/engine.cfg', 'w') as config_file, io.StringIO() as buffer:
        conf1.write(buffer)
        buffer.seek(0)
        buffer.readline()
        shutil.copyfileobj(buffer, config_file)

    with open('./dataset_rush/' + city_name + '/engine.cfg', 'a') as config_file, io.StringIO() as buffer:
        conf2.write(buffer)
        buffer.seek(0)
        buffer.readline()
        shutil.copyfileobj(buffer, config_file)
    ...
    inter = pd.read_json(f'./pre_known_data/log_rush/{city_name}/point_num/0.0%known.json')
    ...
    t2 = time.time()        
    dir_name1 = f'./log_rush_info/increse{args.rate}-webster{args.wb_rate}/'
    ...
    # recording the number of vehicles coming out from each intersection
    '''
    dir_name2 = f'./log_rush_pointout/increse{args.rate}-webster{args.wb_rate}/'
