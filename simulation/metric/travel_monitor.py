...
class TravelMonitor():
    ...
    def total_query(self):
        ...
        for vehicle in list(self.vehicle_enter_time):
            ...
            if road_id in self.road_car_v.keys():

                if vehicle in self.mark_in_road.keys():
                    if current_time > self.in_road_time[vehicle]:
                        self.road_car_v[road_id] += (self.carrun[vehicle] - self.in_road_dis[vehicle]) / (
                                    current_time - self.in_road_time[vehicle])

                        self.road_car_n[road_id] += 1
                else:
                    if current_time > self.vehicle_enter_road_time[vehicle]:
                        self.road_car_v[road_id] += self.carrun[vehicle] / (
                                current_time - self.vehicle_enter_road_time[vehicle])
        ...
        for road_id in self.road_car_v.keys():

            query_set[road_id]= self.road_car_v[road_id]/self.road_car_n[road_id]
            ...
            if self.carwhere[vehicle] == road_id:
                ...
                if vehicle in self.mark_in_road.keys():
                    if current_time>self.in_road_time[vehicle]:
                        ...
                        q_v += (self.carrun[vehicle]-self.in_road_dis[vehicle])/(current_time-self.in_road_time[vehicle])

                        q_n += 1
                else:
                    if current_time>self.vehicle_enter_road_time[vehicle]:

                        q_v += self.carrun[vehicle]/ (
                                      current_time - self.vehicle_enter_road_time[vehicle])
        ...
        for vehicle in vehicles:
            ...
            if not vehicle in self.vehicle_enter_time:
                ...
                if self.carwhere[vehicle] != veh_info['road'][0]:
                    ...
                    if vehicle in self.mark_in_road.keys():
                        car_v = (self.carrun[vehicle] - self.in_road_dis[vehicle]) / (current_time - self.in_road_time[vehicle])
                        del self.mark_in_road[vehicle]
                    else:
                        car_v = self.carrun[vehicle] / (current_time - self.vehicle_enter_road_time[vehicle])

                    if road_id in self.road_car_v.keys():
                        self.road_car_v[road_id] += car_v
                        self.road_car_n[road_id] += 1
                    else:
                        self.road_car_v[road_id] = car_v
                        self.road_car_n[road_id] = 1
